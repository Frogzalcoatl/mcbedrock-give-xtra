...
import { stringToNumber } from "../../prettyTypeId";
...
function getMaxAmount(context: GetStartedContext): number {
	...
	if (
		context.json.slotId !== undefined ||
		(context.json.slot !== undefined && !multiStackSlots.includes(context.json.slot))
	) {
...
function getForm(context: GetStartedContext, maxAmount: number, error?: string): ModalFormData {
	...
	form.textField(formatLabel(top, bottom, error), "", { defaultValue: `${context.json.amount}` });
	form.divider();
	form.label("§r");
...
export async function getStartedAmount(context: GetStartedContext): Promise<void> {
	...
	let form: ModalFormData = getForm(context, maxAmount);
	let input: number | undefined;
	while (input === undefined || input < 1 || input > maxAmount || !Number.isInteger(input)) {
		if (input !== undefined) {
			form = getForm(context, maxAmount, `Invalid amount "${input}"`);
		}
		...
		input = stringToNumber(resp.formValues[0]);
	}
